...
- OpenCode slash commands, agents, and `dot` git subcommands self-document (frontmatter, `dot --help`) and are catalogued in the generated docs reference (`reference/{commands,agents}.md`, `dot/commands.md`); don't re-document them here. The global AGENTS.md routes the behavioural entrypoints (`/plan`, `/grill`, `@commit`, `@commit-push`, `/inject-context`, `/code-review`).
