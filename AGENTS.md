...
- Keep Renovate update grouping limited to standard presets except for the coordinated OpenCode 2 packages in `renovate.json`. Do not add other repository-defined `groupName` rules.
...
- Update flow: edit and validate the owning repository -> authorised source commit and push -> refresh the CLI import in skills -> validate and commit/push skills. Renovate handles this repository's `agents/.agents/skills` submodule revision separately; normal dotfiles updates sync and stow the recorded revision.
...
- When changing `dot` commands, subcommands, aliases, or flags, rebuild and run `dot stow` to refresh local completions for every supported shell.
- The `docs/` command reference (`docs/src/content/docs/dot/commands.md`) is generated from `dot/src/cli/spec.ts`. After changing commands, regenerate it with `mise run docs:gen:cli` and include that reference in the changeset; generated shell completions remain untracked.
...
- Generated catalogues stay generated: `docs/src/content/docs/dot/commands.md` from `dot/src/cli/spec.ts`, and `docs/src/content/docs/agents/opencode/{agents,commands,plugins}.md` from OpenCode assets. Skills are catalogued in the [timmo001/skills](https://github.com/timmo001/skills/blob/main/SKILLS.md#skills-catalogue) `SKILLS.md`, not a local generated page. After those sources change, regenerate with `mise run docs:gen` and commit the result; never hand-edit the generated pages. CI (`docs-drift`, `dot-build`, `opencode-publish`) fails on stale generated output.
...
- OpenCode slash commands and `dot` git subcommands self-document (command frontmatter, `dot --help`) and are catalogued in the generated docs reference (`opencode/commands.md`, `dot/commands.md`); don't re-document them here. The global AGENTS.md routes the behavioural ones (`/plan`, `/grill`, `/commit`, `/inject-context`, `/code-review`).
