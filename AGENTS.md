...
- Git panel PR tracking is opt-in through `pull_requests.enabled` in private `dot-git.yml`; `dot git-pull-requests` owns polling and local seen/delivery state.
...
- `dot deps service` runs separately from `dot update`, through the `dot-deps` user service and timer. Its repository list lives in optional private `scripts/.config/dot/dependency-service.json`; policies must be published before enabling the timer. Dedicated `dot-deps-state` branches hold cross-machine ownership and interval state.
