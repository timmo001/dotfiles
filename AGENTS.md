...
- The `opencode-publish` workflow regenerates and commits the OpenCode reference pages when the assets change; `tui-build` regenerates and commits the command reference when `dot/src/cli/spec.ts` changes, and also builds the docs site (link validation) on `docs/**` or `dot/**` changes.
