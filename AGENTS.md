...
- `agents/.agents/skills/` is the `timmo001/skills` submodule and exposes its skills via `~/.agents/skills/`. Create or change every reusable skill in the standalone checkout under `~/repos/skills`, even when first requested from this repo, then update the pinned revision here after that repository revision is committed and pushed.
