...
- CI enforces the generated pages both ways: the `docs-drift` workflow fails a pull request when `mise run gen` would change `dot/commands.md` or `reference/*.md`, so regenerate and commit before opening a PR. On direct pushes to the default branch, `tui-build` regenerates and commits the command reference and `opencode-publish` does the same for the OpenCode reference, and `tui-build` also builds the docs site (link validation) on `docs/**` or `dot/**` changes. Hand-written pages are not auto-checked, so the mapping above is on you.
