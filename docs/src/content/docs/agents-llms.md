...
- For repository-specific work, pair these docs with current branch context from [`dot git-context`](/git/context/) and tech-stack context from [`dot stack-context`](/dot/stack-context/). The docs explain the workflow; the commands explain the live checkout.
