...
```bash
run-in-terminal --wait 'dot workspace-restore --dry-run'
...
```bash
run-with-notify 'Capture' dot workspace-capture
run-with-notify -a 'Workspace' 'Restore' dot workspace-restore
```
...
A second `show` on the running overlay updates the message. `dot workspace-setup` uses that to follow the current setup step; see [Workspace Setup](/knowledge-base/workspace-setup/).
