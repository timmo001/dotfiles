...
## Toggle HDR on the right monitor

`toggle-right-monitor-hdr` temporarily enables 10-bit HDR with the full BT.2020 colour gamut on `HDMI-A-2` and raises SDR content brightness through Hyprland's `sdrbrightness` setting (currently `2.0`):

```bash
toggle-right-monitor-hdr
```
