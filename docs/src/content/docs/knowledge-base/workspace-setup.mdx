...
Setup shows the [`popup-loading`](/knowledge-base/desktop-command-wrappers/#loading-overlay) overlay on the focused monitor while it runs. The overlay starts with `Setting up workspace...` and then follows the current high-level step, such as preparing apps or rebuilding a split tree. Window moves, tags, and other slot details stay in the log. The overlay hides when setup finishes or fails.
