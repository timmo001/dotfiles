...
Terminal slots launch through `ghostty-host-config`, so they keep the active `config.$OMARCHY_HOST` override. Work-time terminal slots also pass the Home Assistant frontend checkout as `--working-directory`.
