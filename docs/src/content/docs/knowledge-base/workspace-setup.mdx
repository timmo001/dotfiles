...
| `2` | Herdr focused on its Home Assistant Frontend workspace. |
| `3` | Work browser. |
...
Terminal slots launch through `ghostty-host-config`, so they keep the active `config.$OMARCHY_HOST` override. The left monitor's terminal always starts in the home directory. The central monitor runs `herdr-work` during work time and attaches to the shared default Herdr session outside work time. `herdr-work` also uses the default session, then creates or focuses the Frontend workspace before attaching its client.
