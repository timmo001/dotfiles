...
Type to filter every visible action and row. Use Enter on a repository to open its action list. Enter on a TUI action such as lazygit opens it in the repository's Herdr workspace, while Shift+Enter opens it in a floating terminal. Use Up/Down to move, Backspace to edit the filter or return to the previous page, Escape to clear or close, and Tab to move to the next bar panel.
