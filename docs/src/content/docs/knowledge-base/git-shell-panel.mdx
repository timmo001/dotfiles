...
The overview lists changed repositories, an Other repositories link, filtered GitHub notification threads, and a footer link to the GitHub inbox. Selecting a repository opens lazygit, an editor, an agent launcher, a terminal, or GitHub. **Open in agent** lists agents whose Herdr integrations are installed (OpenCode 2 first when present, then OpenCode 1 and other integrations such as Cursor or Pi). Editor, agent, and terminal actions reuse a matching Herdr workspace through `herdr-repo-open` when the shared Herdr server is running; without Herdr they open an ordinary tiled terminal.
