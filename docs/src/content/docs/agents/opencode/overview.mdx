...
| Piece | Why |
| --- | --- |
| `agents/.config/opencode/` | Agents, commands, `plugins/` (OpenCode 1), `plugins-v2/` (OpenCode 2), and shared `lib/` |
...
| `dot mcp-sync` | Regenerate MCP configs from private `mcp.yml`; OpenCode repos opt in via `opencode_mcp` in private `dot-git.yml` |
...
Sessions often run inside Herdr panes (OpenCode integration checked by `dot doctor`). The public Zsh config aliases `opencode` to OpenCode 2 (`opencode2`). Companion harnesses [Pi](/agents/pi/) and [Cursor](/agents/cursor/) share skills and/or AGENTS guidance but not this full plugin and command stack. See [Agents](/agents/overview/) for how the harnesses fit together.
