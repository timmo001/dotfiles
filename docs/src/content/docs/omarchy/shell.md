...
`timmo.notes-capture` opens a keyboard-first multiline capture panel on the primary output. Ctrl+Enter submits to `notes capture` through the private `notes-capture-local` adapter; Escape closes the panel. The searchable picker is generated from notification-enabled private repositories, and Automatic leaves repository selection to the Notes agent. The panel checks the local OpenCode service every 15 seconds, disables Send while it is unavailable, and preserves the draft after validation, availability, or processing failures. It never opens or submits to the hosted web capture. Its IPC target is `timmo.notes-capture` with `open`, `close`, `show`, `hide`, and `toggle` methods.
