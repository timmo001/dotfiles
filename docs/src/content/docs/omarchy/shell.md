...
`timmo.home-assistant` keeps the HA pollers and singleton streams in one shell service. `Config.qml` owns the desktop and laptop entity mappings, commands, actions, aggregate show conditions, labels, icons, colours, and panel sizing; the other QML files only run and render that configuration. The laptop air-conditioner target appears below 26 °C, while the desktop target appears when its enabled input is on. The value is blue while the matching climate entity is cooling, faded blue otherwise, and hidden when that climate entity is unavailable. Desktop also ends the panel with an Office curtains status row and a compact left-to-right grid for the Close, 10%, 20%, 30%, and 60% presets on `cover.curtain`; these panel-only actions do not affect the bar and are omitted on laptop. Clicking Calendar or a sensor opens its existing floating full view; Time check and In a call retain their direct toggle actions. The doorbell stream stays loaded without a visible row and continues opening the camera popup on an active transition.
