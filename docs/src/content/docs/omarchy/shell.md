...
- **Idle timers**: screensaver at 30 minutes, lock at 60 minutes.
...
The personal status widgets read from `dot` bar output and Home Assistant. See [Bar Integrations](/bar-integrations/) for the `--bar-json` commands behind the git, workflow, and notification cells.
