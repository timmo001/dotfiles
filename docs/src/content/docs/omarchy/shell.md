...
`timmo.home-assistant` keeps the HA pollers and singleton streams in one shell service. `Config.qml` owns the desktop and laptop entity mappings, commands, actions, aggregate show conditions, labels, icons, colours, and panel sizing; the other QML files only run and render that configuration. The host-specific target-temperature reading keeps its desktop/laptop visibility gates and cooling colour in the bar, but is omitted from the Environment section because the Controls section already provides its editor. The Controls section separately groups climate status, fan mode, target editing, and enabled controls under one Air conditioner subheading without repeating that name on each row. The climate status row shows `Cool` while cooling, followed by its labelled fan mode and target temperature separated by bullets, opens Home Assistant more-info, and puts Low (`1`) and High (`2`) fan actions at the top of the subsection; the row and actions are omitted while the host's climate entity is unavailable. The target editor has decrement and increment buttons using the helper's configured step. Laptop starts Controls with left, middle, and right living-room blind rows using `cover.set_cover_tilt_position` presets at 0%, 20%, 40%, 60%, 80%, and 100%, followed by a second air-conditioner row of presets ordered 22.6 °C, 22.8 °C, 23.2 °C, 23.4 °C, 23.6 °C, 23.8 °C, 24.2 °C, 24.4 °C, and Off (36 °C). Desktop instead starts Controls with an Office curtains status row and a compact left-to-right grid for the Close, 10%, 20%, 30%, and 60% presets on `cover.curtain`, followed by an enabled toggle below the target; these panel-only actions do not affect the bar. Clicking Calendar or a sensor opens its existing floating full view; Time check and In a call retain their direct toggle actions. The doorbell stream stays loaded without a visible row and continues opening the camera popup on an active transition.
