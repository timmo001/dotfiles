---
title: Controls
description: The Omarchy controls menu and local keybindings.
sidebar:
  order: 3
---
...
Open the upstream Omarchy controls menu with the standard keybinding or command:

```bash
omarchy-menu             # same as Super+Alt+Space
```
...
| `SUPER+CTRL+TAB` | Former workspace | Open the [workspace setup](/knowledge-base/workspace-setup/) picker (Normal, then Work) |
...
`CTRL+ALT+N` toggles the Notes panel from the Omarchy bar.
