...
`timmo.notes` is published from [`timmo001/notes`](https://github.com/timmo001/notes/tree/dev/omarchy-plugin) into [`timmo001/omarchy-notes`](https://github.com/timmo001/omarchy-notes), which dotfiles pins as a managed submodule. Its keep-loaded service backs a primary-output bar widget and keyboard-first panel. The overview searches every repository with Notes' ranked search and opens separate Notes and Handoffs lists. Both lists can filter by repository, tag, and priority; sort by modified time or name in either direction; and group by repository, priority, or not at all. A note detail view renders its metadata and Markdown, then provides guarded native editing, external editing, priority changes, repository moves, and confirmed deletion. The panel can also create notes and handoffs directly.
