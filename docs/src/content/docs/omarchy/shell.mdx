...
Twitch, Git, and Home Assistant share the same quick-filter controls. Typing filters their visible actions and rows. Up and Down move through matches, Enter activates one, Backspace edits the filter, Ctrl+Backspace removes a word, Ctrl+U clears it, and Escape clears an active filter before closing the panel. In the Git panel, Backspace with an empty filter returns to the previous page.
