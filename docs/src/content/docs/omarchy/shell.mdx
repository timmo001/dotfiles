...
- **System Bridge and Notes**: `timmo.system-bridge` sits directly after Home Assistant, followed by `timmo.notes`.

Secondary outputs keep the core menu, `timmo.workspaces`, `timmo.clock`, and built-in system widgets; Twitch, Git, command cells, Home Assistant, System Bridge, and Notes collapse without starting per-output pollers or loading their panels.
...
| `timmo.notes` | service, bar-widget | Searches, reads, creates, edits, moves, prioritises, deletes, and captures repository notes and handoffs. |
...
`timmo.git` polls `dot git-diff --bar-json` and `dot git-notifications --bar-json` once per minute through one shell service, and loads `dot git-diff --panel-json` when its native panel opens. Each source appears only while its count is above zero, so a clean source contributes neither an icon nor a count. Important notifications are red, ordinary changes or unread notifications amber, pull-only repositories green, private-only dirt blue, and unavailable state grey. When both sources are clear, the widget collapses and reveals both bare icons dimmed while hovering the bar or while its panel is open. Left click opens the filterable panel with changed repositories, an Other page, notifications, and direct repository actions. Searching from the overview includes every tracked repository, while searches in subviews remain limited to that subview. `dot git-notifications` opens the panel directly on its notifications page, where the final item after a separator opens the GitHub inbox. Lazygit uses Enter for a new Herdr pane, `Ctrl+Enter` for a focused Lazygit tab, and `Shift+Enter` for a short-lived floating terminal. Open in agent lists agents whose Herdr integrations are installed and opens the selected one in a focused named tab. OpenCode 2 and OpenCode 1 are separate options, with OpenCode 2 first and launched through its isolated wrapper. Editor, agent, and terminal actions use `dot herdr-repo-open` to focus a matching Herdr workspace when available or a normal tiled terminal otherwise. If the shared Herdr server is running without an attached terminal, the command opens the default Herdr terminal and waits for Herdr to acknowledge its foreground client before focusing the workspace. Right click refreshes repository and notification state.
...
`timmo.notes` is published from [`timmo001/notes`](https://github.com/timmo001/notes/tree/dev/omarchy-plugin) into [`timmo001/omarchy-notes`](https://github.com/timmo001/omarchy-notes), which dotfiles pins as a managed submodule. Its keep-loaded service backs a primary-output bar widget and keyboard-first panel, toggled with `CTRL+ALT+N`. The overview searches every repository with Notes' ranked search and opens separate Notes and Handoffs lists. Both lists can filter by repository, tag, and priority; sort by modified time or name in either direction; and group by repository, priority, or not at all. A note detail view renders its metadata and Markdown, then provides guarded native editing, external editing, priority changes, repository moves, and confirmed deletion. The panel can also create notes and handoffs directly.

Open in agent mirrors the detected Herdr integrations and ordering used by `timmo.git`: OpenCode 2, OpenCode 1, Pi, Cursor Agent, Claude Code, Codex, GitHub Copilot, OMP, Devin, Droid, Kimi, Kilo, Hermes, Qoder CLI, Qwen, Mastra Code, Antigravity CLI, and Grok. Only installed integrations are shown. The selected agent opens in the note's recorded repository checkout and receives the full note body and metadata as loaded context.

Capture is a subview of the same panel. `CTRL+ALT+C` runs `omarchy-shell timmo.notes capture` to open it directly, while selecting the bar widget opens the Notes overview. Ctrl+Enter submits through the private `notes-capture-local` adapter and Escape returns to the overview. Submissions clear the editor immediately and run sequentially, so another note can be entered while processing continues. Automatic leaves repository selection to the Notes agent; the searchable repository targets still come from `${XDG_CACHE_HOME:-$HOME/.cache}/dot/notes-capture-repositories.json`. The panel checks the local processor every 15 seconds, disables Send while it is unavailable, restores unsaved text from `notes-capture-draft.txt`, and saves the latest failed submission to `notes-capture-failed-draft.txt` before raising a desktop notification. It never submits to the hosted web capture. The `timmo.notes` IPC target provides `open`, `close`, `show`, `hide`, `toggle`, and `capture`.

`timmo.home-assistant` keeps the HA pollers and singleton streams in one shell service. `Config.qml` owns the desktop and laptop entity mappings, commands, actions, aggregate show conditions, labels, icons, colours, and panel sizing; the other QML files only run and render that configuration. The host-specific target-temperature reading keeps its desktop/laptop visibility gates and cooling colour in the bar, but is omitted from the Environment section because the Controls section already provides its editor. The Controls section starts with Air conditioner and separately groups climate status, target editing, and enabled controls without repeating that name on each row. The climate status row shows `Cool` while cooling, followed by its labelled fan mode and target temperature separated by bullets, opens Home Assistant more-info, and puts Low (`1`) and High (`2`) fan actions inline; the row and actions are omitted while the host's climate entity is unavailable. The target editor has decrement and increment buttons using the helper's configured step, while its label and the desktop enabled-control label open their Home Assistant more-info windows. Laptop includes a second row of presets ordered 22.6 °C, 22.8 °C, 23.2 °C, 23.4 °C, 23.6 °C, 23.8 °C, 24.2 °C, 24.4 °C, and Off (36 °C). A sofa-labelled Blinds subsection follows with left, middle, and right living-room rows showing their state and current tilt position and using `cover.set_cover_tilt_position` presets ordered 100%, 80%, 60%, 40%, 20%, and 0%. Desktop instead has an enabled toggle below the target, followed by an Office curtains status row and a compact single-row grid for the Close, 10%, 20%, 30%, 40%, 50%, and 60% presets on `cover.curtain`; these panel-only actions do not affect the bar. Clicking Calendar or a sensor opens its existing floating full view; Time check and In a call retain their direct toggle actions. The doorbell stream stays loaded without a visible row and continues opening the camera popup on an active transition.

Twitch, Git, and Home Assistant share the same quick-filter controls. Typing filters their available actions and rows. Up and Down move through matches, Enter activates one, Backspace edits the filter, Ctrl+Backspace removes a word, Ctrl+U clears it, and Escape clears an active filter before closing the panel. In the Git panel, Backspace with an empty filter returns to the previous page.
...
`omarchy plugin add` keeps Omarchy's normal review and enable prompts, then asks where a bar widget belongs. After choosing left, centre, or right, choose an existing neighbour and whether the new widget goes before or after it. The lifecycle hook forwards the validated checkout through the thin `manage-omarchy-plugin` compatibility wrapper to `dot omarchy-plugin add`. The command imports it as a submodule under `omarchy/.config/omarchy/plugins/`, records its placement in `omarchy-plugins.json`, and runs `dot stow`.

The parent repository pins the exact plugin SHA. `.gitmodules` records the upstream URL and branch or tag, normalising GitHub SSH URLs to HTTPS so external build systems can clone every submodule, and Renovate proposes newer pins. `omarchy plugin update` and `dot omarchy-plugin update` move managed pins after validation and roll back a failed validation; remove commands delete the submodule and placement declaration. Removal clears and checks the source, live link, module cache, local Git registration, `.gitmodules` entry, and registry before reporting success. After an interactive add or remove, choose **No** (the default), **Commit**, or **Commit and push**. The add prompt puts **Discard plugin** first, while keeping **No** selected by default; discarding immediately removes the plugin and its managed configuration. Non-interactive runs leave ordinary unstaged dotfiles changes. Exit code 20 tells the lifecycle hook that a plugin is unmanaged so Omarchy can retain its stock update and remove behaviour.
...
An optional `omarchy-plugins.json` in the private dotfiles overlay extends this registry. Private entries override public entries with the same plugin ID, and `dot update` restores pinned submodules in both repositories.
Set `enabled` to `false` on a managed entry to retain its source, placement, and settings without adding it to generated `shell.json`.
