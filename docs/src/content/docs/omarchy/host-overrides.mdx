...
Ghostty-only bindings (active while Ghostty has keyboard focus):

| Binding | Action |
| --- | --- |
| `CTRL+ALT+TAB` | Send tmux's next-window sequence |
| `CTRL+ALT+SHIFT+TAB` | Send tmux's previous-window sequence |
| `CTRL+ALT+SHIFT+G` | Type and run `lazygit` |

Hyprland captures `CTRL+ALT+SHIFT+G` globally for the Git panel's Other repositories view before Ghostty can type `lazygit`. Use `CTRL+ALT+G` for the unified Git panel, or run `lazygit` from a shell when you want it inside Ghostty.
