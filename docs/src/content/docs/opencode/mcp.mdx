...
| `system-bridge` | remote | System Bridge: system info modules (cpu, memory, disks, media) and media control. |
...
The `system-bridge` server connects directly to the machine's LAN Streamable HTTP endpoint. Agent harness wrappers load its bearer token with `system-bridge client token`, so the machine-specific address and credential are not published in the shared config.
...
:::note[Credentials]
`context7`, `github`, and `system-bridge` need credentials, supplied through the environment and referenced from `opencode.json` with `{env:...}` so no secret ever lands in the config file. The GitHub server reads `DOT_GH_MCP_BEARER`, and System Bridge reads `SYSTEM_BRIDGE_MCP_BEARER`. The `opencode`, `cursor`, `code`, and `copilot` shell wrappers and `opencode-server` obtain them only for the launched harness process, rather than exporting them to every shell child. `dot doctor` warns when `gh` is missing or has no token.
:::
...
This gate currently runs only in OpenCode V1. The pinned v2 Effect API exposes `context.mcp.transform`, but its callback receives only an `MCPDraft`. It provides no project, session, or directory location, so the plugin cannot preserve marker lookup from the active project without falling back to process-global location.
...
```json
{
  ...
  "mcp": {
    ...
    "system-bridge": {
      "type": "remote",
      "url": "http://192.168.1.10:9170/api/mcp",
      "oauth": false,
      "headers": {
        "Authorization": "Bearer {env:SYSTEM_BRIDGE_MCP_BEARER}"
      }
    }
  }
}
