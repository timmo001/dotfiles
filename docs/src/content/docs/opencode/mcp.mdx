...
| `fallow` | local | Codebase intelligence for JavaScript and TypeScript: unused code, duplication, circular deps, complexity. Opt-in, held out of default tools (see below). |
...
It runs only when invoked by name or through a `/fallow-*` command.

**Per-repo servers.** Some servers are only useful in repos with a matching marker file. The `mcp-repo-gate` plugin prunes them from the merged config at startup when the marker is absent, so their tools do not load where they are irrelevant. For example, `pitchfork` is dropped in any repo without a `pitchfork.toml`, and kept in repos that have one:
...
The plugin is deliberately conservative: only servers listed in that map are ever removed, and only when the marker is missing from the project directory up to the filesystem root. Every other server stays on everywhere, so no tool is hidden where it might be wanted.
