...
| `dot` | local | Repository notes vault (`note_read`, `note_list`, `note_write`, `note_delete`) plus read-only `dot://` resources. Runs `dot mcp`. |
...
The `context` server runs `context mcp` from the `context-git` AUR package (also in `.dot-public-packages`). It is separate from `dot mcp`: repository context tools and schemas live entirely in the standalone [`context`](https://context.timmo.dev) project. See [Context Integration](/git/context/) for how OpenCode plugins consume its JSON output.

The `dot` server exposes only the notes vault and dot-owned read-only resources. Generic git/stack context moved to `context mcp`; see [the dot Command MCP Server](/dot/mcp/).

:::note[Credentials]
`context7` and `github` need credentials, supplied through the environment and referenced from `opencode.json` with `{env:...}` so no secret ever lands in the config file. The GitHub server reads `DOT_GH_MCP_BEARER`, which shell startup typically exports as `$(gh auth token)`. `dot doctor` warns when `gh` is missing or has no token.
:::
...
The `dot` binary also ships its own MCP server for the notes vault. Generic repository context tools live in the standalone `context` server above. See [the dot Command MCP Server](/dot/mcp/) and [Context Integration](/git/context/) for the split.
