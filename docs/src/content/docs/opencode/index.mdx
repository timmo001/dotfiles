...
- `mise run benchmarks:opencode -- --model <provider/model>` - run the opt-in scope benchmark against real files pinned from this repository. Each scenario receives a fresh repository with one synthetic baseline commit and no upstream history, isolated OpenCode config and state, and no model-facing network, fetch, shell, external-directory, or delegation permissions. Deterministic checks audit changed paths, implementation content, findings, and recorded tool calls; a separate read-only model report is advisory. Checksummed, write-protected run evidence is retained under the ignored `.benchmarks/output/opencode/` path. The benchmark uses provider credentials and is intentionally not part of CI.
