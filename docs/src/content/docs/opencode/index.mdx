...
- `dot skill-updates` / `dot skill-check`: check/apply upstream skill updates and validate branch-context command wiring. `dot skill-updates --json` provides a side-effect-free report, while `--update --skill <name> --no-commit` applies one clean import for external automation.
