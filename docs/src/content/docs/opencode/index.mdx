...
The repo ships a shared [OpenCode](https://opencode.ai) configuration: agents, slash commands, skills, and lifecycle plugins. The source lives in `agents/.config/opencode/` (agents, commands, plugins, and shared `lib/` modules) and `agents/.agents/skills/` (skills shared by OpenCode and Codex via `~/.agents/skills/`).
...
- **Staged implementation** - `/plan` keeps broad work to one coherent, independently verifiable stage at a time. It defines shared contracts before delegation, uses targeted checks while iterating, runs one focused review before full validation, and pauses at a validated checkpoint before starting a separate review unit unless the user requested one combined delivery.
- **Branch context injection** — the `branch-context` plugin pre-computes git/PR state once per command and injects it as structured XML, so commands do not run their own `git`/`gh` calls. It consumes the standalone [`context`](https://context.timmo.dev) CLI (`context git --json`), preserves explicit unresolved default-branch state, and keeps the plugin-specific injection rules here.
...
- **Contextual notifications** - top-level task completion and permission events send a desktop notification without taking focus, ring BEL, and play the freedesktop message sound when `paplay` is available. Clicking a notification focuses its originating OpenCode window under Hyprland. Background subagent completions stay silent.
