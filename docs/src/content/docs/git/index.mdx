...
`dot` includes git and GitHub tools that work across every managed repo (public and private dotfiles, configured private git repos, and Omarchy repos), and integrates the standalone `context` CLI for branch and stack context. GitHub notifications open in the Omarchy shell, with `--raw` and `--bar-json` output available for scripts and status bars. Local commit automation goes through the guarded `dot git-commit` gateway.

<CardGroup cols={2}>
  <Card title="Diff & Repo Watcher" href="/git/diff/">CLI and native shell panel state for dirty worktrees and ahead/behind state.</Card>
