...
`plugins/config/herdr-lazy/plugins.list` and `plugins.lock` pin third-party plugins. When a Herdr session is available, `dot update` restores them through `herdr-lazy`. Stowed companion config covers sessionizer, usagebar, and reviewr. Everything else under `~/.config/herdr/` remains runtime-owned.
...
| `dot herdr` | Start, stop, or restart the shared server; open a repo workspace; show attached context |
