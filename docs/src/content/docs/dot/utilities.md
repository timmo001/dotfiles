...
`dot launch-floating-webapp <url>` opens one webapp as a `380x500` floating window in the focused monitor's bottom-right corner without changing normal launches of the same site. A temporary class-specific Hyprland rule makes the window floating as soon as it maps, then disables itself before final placement. Use `--monitor`, `--workspace`, `--width`, `--height`, `--right-margin`, or `--bottom-margin` to override the geometry. `--address` applies the same placement to an existing window. The command prints only the resolved Hyprland address to stdout, so callers can capture it directly.
