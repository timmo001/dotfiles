...
It is safe to rerun. If `herdr` is not on `PATH`, the command reports a skip instead of failing bootstrap. `dot init` and full `dot update` runs call it automatically.
