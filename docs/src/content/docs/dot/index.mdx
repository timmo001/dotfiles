...
`dot dashboard` opens a full-screen Mission Control dashboard. It combines tracked repo state, GitHub notifications, workflow runs, and selected bounded status-bar sources such as Twitch, environment, and calendar modules. Cards are intentionally readable and always present; Enter opens an existing deeper TUI view when a card has one.
