...
`dot` is the compiled CLI for these dotfiles ([Bun](https://bun.sh) + [Effect](https://effect.website) v4), stowed to `~/.local/bin/dot`. It owns setup, sync, stow, health checks, desktop helpers, and the git tooling used day to day.
...
Managed-repo lists and notification rules live in the private `dot-git.yml` overlay. Branch and stack context for agents come from the separate [`context`](https://context.timmo.dev) tool.
