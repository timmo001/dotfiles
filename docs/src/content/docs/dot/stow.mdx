...
- When Omarchy is enabled, enables and restarts `dot-reload-ui-monitor.service` after the public stow so resume recovery keeps watching logind. See [Resume recovery](/knowledge-base/resume-recovery/) for manual checks and the `reload-ui` shortcut.
