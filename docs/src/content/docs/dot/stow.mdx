...
The repository is a [GNU Stow](https://www.gnu.org/software/stow/) package root targeting `~/`. Each top-level directory (`zsh`, `neovim`, `starship`, `hypr`, `ghostty`, `scripts`, ...) is a stow package whose contents are symlinked into your home directory.
...
- Stows any package that targets runtime-owned directories with `--no-folding`, including `hypr/`, `ghostty/`, shell completions, `.local/bin`, and systemd user units. For Hypr it also creates/repairs the `~/.config/hypr/host` symlink for the active host.
- Before stow, moves unmanaged real files or directories that block active package targets into the repo's `backup/` directory and logs each source-to-backup path. This includes host-specific packages such as `chromium--laptop`.
...
- During `dot install` and `dot init`, adopts existing files where needed so a fresh machine does not clobber stock config.
