...
`dot git-diff` status scans disable Git's optional index locks. Background bar and native panel polling can read working-tree state during a rebase, merge, or another index-writing operation without refreshing the index or competing for `.git/index.lock`.
