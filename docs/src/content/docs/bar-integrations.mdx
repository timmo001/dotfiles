...
| Mode | Behaviour |
| --- | --- |
| `temperature` | Show a signed numeric temperature reading with its Celsius unit. Add `--show-above <number>` to hide it unless the reading is strictly above that threshold, or `--icon-only` to keep the reading in the tooltip. |
...
`dot dashboard` rejects commands that embed `singleton-stream` or `doorbell`, so use `ha-entity-bar-json-once` for dashboard cards and reserve `ha-module-bar doorbell` for a streaming bar widget. See [Private Dashboard Config](/configuration/private-dashboard/).
