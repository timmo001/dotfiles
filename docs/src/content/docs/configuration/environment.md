...
The global mise configuration (`mise/.config/mise/config.toml`) pins the shared developer toolchain, including the Skills CLI used alongside `dot skills` and [memex](https://github.com/nicosuave/memex) for browsing and searching local coding-agent history. The matching Herdr plugin provides session palettes, browsing, resume, indexing, and web UI actions. Node.js and Bun versions are also declared in the repository root `mise.toml` for `dot`, `skill-maintenance`, and `docs` tasks. `dot update` compiles `skill-maintenance` from the writable `~/repos/skills` checkout when available, otherwise from the pinned skills submodule, before generating completions and stowing it into `~/.local/bin`.
