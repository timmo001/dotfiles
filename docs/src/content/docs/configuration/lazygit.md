...
Press `Ctrl+F` in the Local Branches panel to run `git-rebase-default`. The command fetches the default branch from `upstream`, falling back to `origin` when no upstream remote exists, then rebases the checked-out branch onto the fetched commit. Run the same command from a shell with the `grd` alias.
