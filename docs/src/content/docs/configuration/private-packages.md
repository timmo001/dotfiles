...
The list covers shared tooling rather than desktop apps you might install separately: build helpers (`cmake`, `gcc`, `pkgconf`), diagnostics (`bmon`, `iperf3`, `sysstat`, `topgrade-bin`), shell and terminal helpers (`zsh` and its plugins, `mise-bin`), desktop integrations (`kdeconnect`, `system-bridge-git`), and other utilities referenced across the dotfiles setup. `context-git` will join this list once the standalone context package is published. Override the path with `DOT_PUBLIC_PACKAGES_FILE`.
