...
:::tip[Herdr config]
`dot init` stows the shared Herdr config and runs `dot herdr-sync` for integrations. Ghostty and zsh still open as plain shells; start Herdr explicitly with `herdr` when you want it.
:::
