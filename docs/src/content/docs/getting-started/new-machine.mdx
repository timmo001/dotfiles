...
6. If a stock Omarchy config directory already exists at `~/.config/uwsm`, `dot init` backs it up with a `.dot-init-backup-*` suffix before cloning the managed repo. Hyprland and Ghostty config are stowed from the `hypr/` and `ghostty/` packages instead.
