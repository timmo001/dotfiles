...
11. Reboot so the Omarchy session picks up `OMARCHY_HOST`, then run `dot doctor` to verify the setup. Use `dot update` for ongoing sync, stow, Herdr plugin refresh, and rebuilds.
...
Local Ghostty zsh shells auto-attach to Herdr when it is installed. Use `DOT_NO_HERDR=1 zsh` for a plain recovery shell. See [Herdr Session](/configuration/herdr/) for the Caps prefix and action palette.
:::
