...
`dot update` is the everyday command. It pulls the public dotfiles, installs Bun dependencies, rebuilds and relaunches on the new binary, then scans and pulls tracked repositories. The remaining full-update phases trust mise configs, regenerate completions, sync MCP configs, stow, rebuild, install Herdr Lazy when needed, restore the declared Herdr plugins from its lockfile, sync agents, backfill the init marker, and refresh resume-managed services. Package setup stays in `dot init`, while `dot doctor` reports package health. See the [Command Reference](/dot/commands/) for scoped phase flags and exit codes.
