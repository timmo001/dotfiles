...
The `tui-build` workflow also builds the docs site (`mise run docs:build`, which runs `starlight-links-validator`) on `docs/**` or `dot/**` changes, so broken internal links fail CI before Cloudflare deploys.
