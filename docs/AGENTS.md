...
The `tui-build` workflow also builds the docs site (`mise run docs:build`) on `docs/**` or `dot/**` changes. Blume's strict build and validation commands catch invalid content and broken links before Cloudflare deploys.
