...
Item {
  ...
  readonly property bool releaseStale: releasesError !== "" || releases.some(function(entry) { return entry.stale })
  signal panelUpdated()
  ...
  function prepareRelease(entry, summary, command, modifiers) {
    ...
    var prompt = [
      ...
      "After confirmation, prepare and validate the selected release. Present the prepared release summary in chat first, including the version, target commit, release title, full description and validation results. Immediately after the summary, use the structured question tool to ask whether to release now or stop with the prepared changes. Name the actual version in the release choice and make clear that it authorises the required commit, push, tag and publication steps. Wait for the answer before taking those steps, then follow the repository's release workflow if approved. The earlier confirmation authorises preparation only; do not end at the summary or leave the user to request publication unprompted.",
