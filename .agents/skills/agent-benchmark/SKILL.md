...
4. Report the daemon name and the status result immediately. Point to `mise run benchmarks:opencode:logs` for current output, `mise run benchmarks:opencode:logs:follow` (or `mise run benchmarks:opencode:logs -- --follow`) for live output, and `mise run benchmarks:opencode:stop` for cancellation.
