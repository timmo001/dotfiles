...
- Stay generated; never hand-edit:
  ...
  - `docs/src/content/docs/agents/opencode/{agents,commands,plugins}.md` ← OpenCode assets (paths under `/agents/opencode/`; sidebar nest under top-level **OpenCode**, with **Other agents** as a sibling group for Pi/Cursor)
- Shared skills are catalogued in <a href="https://github.com/timmo001/skills/blob/main/SKILLS.md#skills-catalogue" target="_blank" rel="noopener noreferrer">timmo001/skills</a> (`SKILLS.md`), not a local generated page. Link there from OpenCode prose first; do not ship or regenerate `reference/skills`.
...
- Keep `blume.config.ts`, Overview, `agents/opencode/overview.mdx`, and README docs-map aligned: **paths** under `/agents/opencode/*` and `/agents/{pi,cursor}`; **sidebar** stays flat top-level **OpenCode** and **Other agents** (same weight as Desktop; do not nest OpenCode under an Agents group, Blume subsection labels look identical). Skills link out to the skills repo (plus the local-skills table on OpenCode); machine-readable docs URLs live on Overview, not a separate Agents/LLMs page; `dot/commands` sits with `dot`. Sidebar labels: `/agents/opencode/overview` → **Overview**, `/agents/opencode/agents` → **Agents** (via generated page title). Desktop pages live under `/desktop/*`.
