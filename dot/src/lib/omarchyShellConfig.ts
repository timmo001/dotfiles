...
import { currentOmarchyHost } from "./omarchyHost.js";
...
function temperatureEntry(host: string): BarEntry {
  ...
  return command({
    run: `ha-bar-module temperature --entity ${entity} --name '${name}' --icon 󰔏 --unit °C`,
    interval: 15000,
...
function diningTemperatureEntry(): BarEntry {
  ...
  return command({
    run: `ha-bar-module temperature --entity ${entity} --name 'Dining Room Temperature' --icon 󰩰 --unit °C`,
    interval: 15000,
...
function co2Entry(host: string): BarEntry {
  ...
  return command({
    run: `ha-bar-module co2-alert --entity ${entity} --name '${name}' --icon 󰟤 --unit ppm`,
    interval: 15000,
...
function doorbellEntry(host: string): BarEntry {
  ...
  return stream({
    run:
      "ha-bar-module doorbell --entity input_boolean.doorbell --icon 󰂚 " +
...
function calendarEntry(): BarEntry {
  return command({
    run: "ha-bar-module current-next-event --entity input_text.current_next_event_in_an_hour --icon 󰃭",
    interval: 30000,
    onClick:
...
function customCenterEntries(): BarEntry[] {
  return [
    ...
    command({
      run: "ha-bar-module nas-activity --icon 󰒋",
      interval: 5000,
...
function customRightEntries(host: string): BarEntry[] {
  return [
    ...
    ...(host === "desktop"
      ...
      : [
          command({
            run: "ha-bar-module voc-alert --quality-entity sensor.apollo_air_1_806d64_voc_quality --value-entity sensor.apollo_air_1_806d64_sen55_voc --name 'Apollo Air 1 VOC' --icon 󰵃",
            interval: 15000,
...
export function mergeOmarchyShellConfig(
  ...
  base.idle = { screensaver: 1800, lock: 3600 };
...
> = Effect.gen(function* () {
  ...
  if (!host) {
    yield* log.info("Skipping Omarchy shell config (OMARCHY_HOST is unset)");
    return false;
  }
