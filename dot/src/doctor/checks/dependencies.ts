...
const TOOLS: readonly ToolDef[] = [
  ...
  { name: "omarchy-pkg-add", purpose: "dot setup installs stow when missing", required: false },
  { name: "omarchy-pkg-aur-add", purpose: "dot init installs missing AUR packages", required: false },
  { name: "yay", purpose: "used by omarchy-pkg-aur-add and AUR version checks", required: false },
  { name: "gh", purpose: "branch discovery + authenticated clone convenience", required: false },
  { name: "gum", purpose: "interactive init questionnaire", required: false },
  { name: "notify-send", purpose: "desktop workflow notifications", required: false },
];
...
export const checkDependencies = Effect.gen(function* () {
  ...
  for (const tool of TOOLS) {
    ...
    if (exit === 0) {
      results.push({ severity: "ok", message: `${tool.name} is available (${tool.purpose})` });
    } else if (tool.required) {
      results.push({ severity: "error", message: `${tool.name} is missing (${tool.purpose})` });
    } else {
      results.push({ severity: "warn", message: `${tool.name} is missing (${tool.purpose})` });
    }
  }
  ...
  if (ghExit === 0) {
    ...
    if (authExit === 0) {
      ...
      results.push({
        severity: "warn",
        message: "gh is installed but not authenticated (branch discovery may be rate-limited)",
      });
    }
  }
