...
const DBUS_SECRETS_SERVICE = "/usr/share/dbus-1/services/org.freedesktop.secrets.service";
...
export const checkSecretService = Effect.gen(function* () {
  ...
  if (kwalletExit === 0) {
    results.push({ severity: "ok", message: "kwallet is installed and provides org.freedesktop.secrets" });
  } else {
    results.push({ severity: "warn", message: "kwallet is missing; this setup expects kwallet to provide org.freedesktop.secrets" });
  }

  // Check gnome-keyring is not installed
  const gnomeKeyringExit = yield* executor.exitCode("pacman", ["-Q", "gnome-keyring"]);
  if (gnomeKeyringExit === 0) {
    results.push({
      severity: "warn",
      message: "gnome-keyring is installed; expected kwallet to be the Secret Service provider on this setup",
    });
  }
  ...
  if (existsSync(DBUS_SECRETS_SERVICE)) {
    ...
    results.push({ severity: "ok", message: "No legacy org.freedesktop.secrets DBus autostart file found" });
  }
  ...
  for (const unitPath of GNOME_KEYRING_UNITS) {
    if (existsSync(unitPath)) {
      ...
      results.push({ severity: "warn", message: `gnome-keyring systemd user unit is present: ${unitPath}` });
    }
  }
  if (!gnomeKeyringUnitFound) {
    results.push({ severity: "ok", message: "No gnome-keyring systemd user units found" });
  }
