...
const FAST_MODEL_TOKENS = new Set(["fast", "flash", "mini", "nano", "haiku", "spark", "turbo"])
...
export class CommitSuggest extends Context.Tag("CommitSuggest")<CommitSuggest, CommitSuggestService>() {}

/** Discover available models via `opencode models --verbose` and pick the best fast one */
function discoverFastModel(): Effect.Effect<DiscoveredModel | undefined, Error> {
  ...
  properties: {
    suggestions: {
      ...
      items: {
        type: "object" as const,
        properties: {
          message: { type: "string" as const, description: "A short commit message" },
        },
        required: ["message"],
      },
      minItems: 5,
  ...
  suggest: (diff, recentCommits) =>
    Effect.tryPromise({
      try: async () => {
        ...
        log(`Raw response text (first 500 chars): ${responseText.slice(0, 500)}`)
...
function extractTextFromParts(parts: ReadonlyArray<Record<string, unknown>>): string | undefined {
...
async function pollForAssistantText(
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    for (const msg of result.data) {
      ...
      const text = extractTextFromParts(msg.parts as Array<Record<string, unknown>>)
      if (text) {
