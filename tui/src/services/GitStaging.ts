...
export interface GitStagingService {
  ...
  readonly getStatus: (repoPath: string) => Effect.Effect<readonly StagedFile[], Error>
  /** Stage a single file via `git add` */
  readonly stageFile: (repoPath: string, file: string) => Effect.Effect<void, Error>
  /** Unstage a single file via `git reset HEAD` */
  readonly unstageFile: (repoPath: string, file: string) => Effect.Effect<void, Error>
  /** Stage all files via `git add -A` */
  readonly stageAll: (repoPath: string) => Effect.Effect<void, Error>
  /** Commit staged changes with the given message */
  readonly commit: (repoPath: string, message: string) => Effect.Effect<void, Error>
  /** Get recent commit messages from a repository */
  readonly getRecentCommits: (repoPath: string, count: number) => Effect.Effect<readonly string[], Error>
}

/** Effect service tag for {@link GitStagingService} */
export class GitStaging extends Context.Tag("GitStaging")<GitStaging, GitStagingService>() {}

/** Run a git command in the given repo directory and return stdout */
function runGit(repoPath: string, args: readonly string[]): Effect.Effect<string, Error> {
...
function runGitVoid(repoPath: string, args: readonly string[]): Effect.Effect<void, Error> {
